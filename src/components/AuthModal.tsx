...
export default function AuthModal({ onClose, onLoginSuccess }: AuthModalProps) {
  ...
  // Firebase: Google Authenticated Popup Intake with automated sandbox and redirect fallback
  const handleGoogleSignInClick = async () => {
    ...
    if (isIframe) {
      // Automatically transition to the Sandbox Google Login view to guarantee instant, painless access inside the iframe!
      setShowGoogleSandbox(true);
      setAuthLoading(false);
      setAuthError("Google Identity popup is blocked inside this sandboxed development iframe. We've loaded the secure Sandbox Google Login below so you can sign in instantly!");
      return;
    }
    ...
    try {
      ...
      if (err?.code === "auth/popup-blocked" || err?.message?.includes("popup") || err?.message?.includes("blocked")) {
        // Fallback to Sandbox Google Login automatically to guarantee access
        setShowGoogleSandbox(true);
        setAuthError("Google Identity popup was blocked. We've activated the Sandbox Google Login below to let you sign in instantly!");
        ...
        setAuthError("This domain is not whitelisted in your Firebase Project! Please add it under Authorized Domains or use the Sandbox Google Login.");
      } else {
        setAuthError(err.message || "Google single sign-in failed. Please use standard credentials or the Sandbox Google Login.");
      }
    } finally {
      setAuthLoading(false);
    }
          ...
          {showForgotPassword ? (
            ...
            <>
              ...
              <div className="border-t border-white/5 pt-4 space-y-3">
                <div className="relative flex justify-center text-[10px] mb-2">
                  <span className="bg-slate-950 px-2.5 text-slate-500 font-mono">OR SECURE GOOGLE IDENTITY</span>
                </div>

                {/* Single Polished Google Sign-In button */}
                <div className="flex flex-col items-center justify-center space-y-2.5 w-full">
                  <button
                    id="google-signin-main"
                    type="button"
                    onClick={handleGoogleSignInClick}
                    disabled={authLoading}
                    className="w-full max-w-[340px] py-2.5 px-3 rounded-xl border border-white/10 hover:border-cyan-500/40 bg-white/5 hover:bg-white/10 text-slate-200 hover:text-white transition-all text-xs font-semibold flex items-center justify-center gap-1.5 shadow-md shadow-slate-950 cursor-pointer disabled:opacity-50"
                  >
                  ...
                  <div className="text-center w-full">
                    <button
                      ...
                      className="text-[10px] text-cyan-400 hover:underline flex items-center justify-center gap-1 mx-auto mt-1 cursor-pointer font-mono"
                    >
